...
export async function fetchGpuListings(
  ...
  const url = `${API_BASE}/v1/gpu/listings${qs ? `?${qs}` : ""}`;
...
export async function fetchGpuListing(listingId: string): Promise<GpuListing> {
  const res = await fetch(`${API_BASE}/v1/gpu/listings/${listingId}`);
...
export async function rentGpu(
  ...
  const res = await fetch(`${API_BASE}/v1/gpu/rent`, {
    method: "POST",
...
export async function rateGpu(
  ...
  const res = await fetch(`${API_BASE}/v1/gpu/rate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
export async function fetchGpuReputation(
  ...
  const res = await fetch(`${API_BASE}/v1/gpu/reputation/${publicKey}`);
